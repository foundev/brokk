...
public class FilePanel implements BufferDocumentChangeListenerIF, ThemeAware {
    ...
    public void applyTheme(GuiTheme guiTheme) {
        assert guiTheme != null : "GuiTheme cannot be null";
        // Apply current theme
    ...
    abstract class AbstractHighlight {
        ...
        AbstractHighlight(AbstractDelta<String> delta) {
            assert delta != null : "Delta cannot be null for AbstractHighlight";
            this.delta = delta;
        }
    ...
    private void setHighlight(int offset, int size,
        ...
        assert highlight != null : "HighlightPainter cannot be null";
    ...
    public void documentChanged(JMDocumentEvent de) {
        assert de != null : "JMDocumentEvent cannot be null";
    ...
    private void installMirroring(Document newPlainDoc) {
        assert newPlainDoc != null : "newPlainDoc cannot be null for installMirroring";
        removeMirroring();
    ...
    private DocumentListener createMirroringListener(Document sourceDoc, Document destinationDoc,
        ...
        assert sourceDoc != null : "Source document cannot be null for mirroring listener";
        assert destinationDoc != null : "Destination document cannot be null for mirroring listener";
        assert guard != null : "Guard cannot be null for mirroring listener";
    ...
    private static void copyText(Document src, Document dst) {
        assert src != null : "Source document cannot be null in copyText";
        assert dst != null : "Destination document cannot be null in copyText";
        try {
    ...
    public static class LeftScrollPaneLayout
        ...
        public void layoutContainer(Container parent) {
            assert parent != null : "Parent container cannot be null in LeftScrollPaneLayout";
    ...
    public SearchHits doSearch() {
        ...
        if (!searchText.isEmpty()) {
            for (int line = 0; line < numberOfLines; line++) {
                ...
                // Ensure text is not null; treat null as empty string for search purposes.
                ...
                while ((index = textToSearch.indexOf(searchTextToCompare, fromIndex)) != -1) {
                    ...
                    if (offset < 0) {
                        // This can happen if line number is out of bounds, though loop should prevent it.
                        // Or if document structure is unusual.
    ...
    public void doPreviousSearch() {
        SearchHits sh = getSearchHits(); // This now ensures non-null
        sh.previous();
        reDisplay();
    ...
    private void scrollToSearch(FilePanel fp, SearchHits searchHitsToScroll) {
        assert fp != null : "FilePanel cannot be null for scrollToSearch";
        if (searchHitsToScroll == null) { // Defensive, though getSearchHits now ensures non-null
            return;
        }
